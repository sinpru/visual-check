	...
	try {
		...
		if (!baseline || !current) {
			return NextResponse.json(
				{ error: 'baseline and current paths are required' },
				{ status: 400 },
			);
		}
		...
		if (
			!baselinePath.startsWith(snapshotsRoot) ||
			!currentPath.startsWith(snapshotsRoot)
		) {
			return NextResponse.json(
		...
		if (
			baselineMetadata.width !== targetWidth ||
			baselineMetadata.height !== targetHeight
		) {
			finalBaselineBuffer = (await sharp(baselineBuffer)
				.extend({
					...
					bottom: Math.max(
						0,
						targetHeight - (baselineMetadata.height || 0),
					),
					right: Math.max(
						0,
						targetWidth - (baselineMetadata.width || 0),
					),
		...
		if (
			currentMetadata.width !== targetWidth ||
			currentMetadata.height !== targetHeight
		) {
			finalCurrentBuffer = await sharp(currentBuffer)
				.extend({
					...
					bottom: Math.max(
						0,
						targetHeight - (currentMetadata.height || 0),
					),
					right: Math.max(
						0,
						targetWidth - (currentMetadata.width || 0),
					),
					background: { r: 0, g: 0, b: 0, alpha: 0 },
				})
				.png()
