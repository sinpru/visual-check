    ...
    for (const frame of body.frames) {
      try {
        ...
        const { width, height } = frame.width && frame.height
          ...
          : await getFrameDimensions(fileKey, frame.nodeId, token);

        const buffer = await fetchFigmaBaseline(
          fileKey, frame.nodeId, token, width, height,
        );
        ...
        console.error(`[api/figma-snapshot] Failed for ${frame.testName}:`, message);
      }
    }
